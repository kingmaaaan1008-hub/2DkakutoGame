...
export const PROJECTILES = {
   ...
   * 巫女の御札（地上の攻撃）。
   *
   * 曲がらない・追わない。撃った高さをまっすぐ横へ走るだけの弾で、
   * 出どころは投げ切ったときの指先（胸の高さ 163）。**しゃがめば下を通る**。
   * 女子高生のレーザーと同じ考え方だが、あちらより遅く、そのぶん近くで撃てる。
   */
  talisman: {
    /** 見た目の大きさ。判定は box で別に持つ。 */
    radius: 22,
    ...
    lifetime: 130,
    /**
     * 場に 2 枚まで。撃ち切ると次が出ないので、
     * 一方的に撒き続けて近づけなくする、という戦い方はできない。
     */
    maxAlive: 2,
    /**
     * 発射位置（足元原点・前方向が正）。attack シートで腕を伸ばし切った
     * コマ（f4）の御札の実測（前方 128〜141 / 高さ 161〜165）の中ほど。
     */
    origin: { x: 134, y: 163 },
    /**
     * 御札 1 枚ぶんの判定。**縦に薄い**ので、しゃがみ（やられ判定 0〜100）の
     * 上を素通りする。ここを下げると屈む意味が無くなる。
     */
    box: { w: 54, h: 26 },
    damage: 48,
    hitstun: 18,
  ...
  /**
   * 巫女の御札（空中の攻撃）。**相手を狙って 3 枚まとめて飛ぶ。**
   *
   * 地上のものと分けてあるのは、同時に出せる数を別々に数えたいから。
   * ひとつの type にまとめると、地上で撒いた 2 枚が上限を埋めて
   * 空中の 3 枚が 1 枚しか出ない、ということが起きる。
   *
   * 狙う向きは技側（`aim`）が決める。ここは「まっすぐ飛ぶ」だけを持つ。
   */
  talismanFan: {
    radius: 20,
    // 3 枚が扇に開いて飛ぶので、地上の 1 枚より少し遅い。
    // 速いと開き切る前に相手へ届いてしまい、扇に見えない
    speed: 9.4,
    turnRate: 0,
    lifetime: 110,
    /** 1 回ぶん（3 枚）だけ。落ちるか当たるまで次は投げられない。 */
    maxAlive: 3,
    /**
     * 発射位置。airattack シートで腕を振り抜いたコマ（f5）の御札の実測
     * （前方 112〜131 / 高さ 202〜214）。跳んで投げるので原点も頭より上にある。
     */
    origin: { x: 124, y: 208 },
    /** 斜めに落ちてくるので、地面に届いたところで消える。 */
    floorY: 8,
    box: { w: 48, h: 26 },
    damage: 44,
    hitstun: 18,
  ...
  /**
   * 巫女の結界の欠片。**弾いたスキルの返し**としてだけ飛ぶ。
   *
   * 自分から撃てる弾ではない。相手のスキルが結界に触れた瞬間、
   * 割れた結界の破片がそのまま相手へ返っていく、という見立てなので、
   * 出どころは腕の先ではなく**自分の体の中心**にしてある。
   *
   * スキルを弾いた見返りなので、こちらもスキル扱い（ガードごと持っていく）。
   * 弾いた相手はたいてい大技の最中で、ガードも回避も間に合わない。
   */
  wardShard: {
    radius: 24,
    // 至近距離で返すものなので速い。相手が硬直から抜ける前に届く
    speed: 12.6,
    turnRate: 0,
    lifetime: 80,
    /** 1 回の返しぶん（3 つ）。 */
    maxAlive: 3,
    /** 結界の中心（腰から胸のあたり）。技側の spawns では上書きしない。 */
    origin: { x: 0, y: 112 },
    box: { w: 56, h: 56 },
    damage: 150,
    hitstun: 30,
  ...
  /**
   * 格闘娘の波動弾。**連打の 4 打目にだけ出る。**
   *
   * このキャラは全キャラで一番リーチが短く（刻み突き 106）、飛び道具も
   * 踏み込みも持たない。波動弾はその唯一の例外だが、**単体では撃てない**
   * ところが他の飛び道具と違う。出すには突きを 3 発振り切る必要があるので、
   * 撒いて近づけなくする使い方ができない（＝間合いを詰める理由は消えない）。
   *
   * 場に 1 発まで。当たるか消えるまで次は出ないので、
   * 連打を回し続けても弾幕にはならない。
   */
  kiBlast: {
    radius: 30,
    /**
     * 速さ。**走り(7.4)より少し速い程度**にしてある。
     * 速くすると「撃ってから追う」が成立してしまい、
     * 短いリーチを自分の足で埋めるキャラでなくなる。
     */
    speed: 8.6,
    turnRate: 0,
    /** ステージ幅 1800 に対して 1100 ほど飛ぶ。端から端までは届かない。 */
    lifetime: 130,
    /** 場に 1 発。撃った弾が当たるか消えるまで、4 打目を回しても次は出ない。 */
    maxAlive: 1,
    /**
     * 発射位置（足元原点・前方向が正）。punch2 シートで後手を伸ばし切った
     * コマ（f3/f4）の拳の実測（前方 78 / 肩の高さ 150）の少し先。
     */
    origin: { x: 92, y: 148 },
    /**
     * 判定。**肩の高さを飛ぶので、しゃがまれると下を通る**
     * （やられ判定は屈むと 0〜100 まで縮む）。
    ...
    pushHit: 5,
    pushBlock: 3,
    /** 打撃の延長なのでガードは通る（スキルではない）。 */
    guardBreak: false,
    destroyOnHit: true,
    style: 'wave',
    /** キャラのテーマ色に寄せた金色。魔法使いの青い弾と見間違えないため。 */
    color: '#ffd06a',
  },
  ...
  boyfriend: {
     ...
     * 速いと呼ばれた時点で詰みになるので、見てから跳べる速さまで落としてある。
     *
     * 逃げ道は跳ぶことだけで、**しゃがんでもくぐれない**（判定の下端が 8 なので
     * しゃがみのやられ判定と重なる）。くぐらせたいなら彼氏を浮かせる必要があり、
     * 走ってくる絵と合わなくなるので、跳ぶ一本に寄せてある。
     */
    speed: 7.5,
    turnRate: 0,
    /**
    ...
    entersOffStage: true,
    /**
     * 判定。足元原点で、腰のあたりまで（地上 8〜148）。
     *
     * **頭までは覆わない。** 跳べる高さはどのキャラも身長ぶん（≒215）しかないので、
     * 人ひとりぶん（〜198）を覆うと、頂点で一瞬だけ上に出られても
     * 横幅 110 を通り抜けるあいだ上に居られない ＝ 跳んでも越えられなくなる。
     * ここを腰までに留めることで、踏み切りに 22 ティックの猶予ができる
     * （総当たりで測った値。198 のときは越えられる踏み切りが 1 つも無かった）。
     */
    box: { w: 110, h: 140, y: 8 },
 ...
 *
 * **踏み込みを空振った時点で判定は切れる**（`spent` が立つ）。滑りと走り去りは
 * 見せるだけで、当たらない。ここを生かしたままにすると、滑って止まるのが
 * ちょうど相手の足元なので、**跳び越したのにその上へ着地して当たる**ことになり、
 * 跳んで避ける手が丸ごと成立しなくなる（実測でも避ける手が 1 つも無かった）。
 *
...
export function stepLunge(p, def) {
  ...
  // 踏み込み切った ＝ この突進は空振り。ここから先の滑りと走り去りは当たらない
  p.spent = true;
