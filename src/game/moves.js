...
const MOVE_DEFAULTS = {
  ...
  landLag: null,
  /**
   * 叩きつけて降りる技の着地。{shake, dust, radius, hold}
   *
   * 指定すると、着地した瞬間に足元へ土煙（`dust` 演出）を出して、
   * 画面を `shake` だけ揺らす。`dust` は土煙の寿命（ティック）、`radius` は広がる幅。
   *
   * `hold: true` を足すと、そのうえで **land の絵に移らず、その技の最後のコマを
   * 硬直の間ずっと保持する**。既定では着地した瞬間に立ち上がりの絵へ移ってしまうので、
   * 「振り下ろした刃が地面に刺さっている」ような**決めの絵を持つ技**用。
   * 決めの絵を持たない技は付けないほうがよく、**ジャンプと同じ着地の絵**へ
   * 素直に移ったほうが立て直しの動きとして繋がる。
   *
   * どちらにしても**硬直の長さには触らない**。長さを決めるのは `landLag` で、
   ...
   *
   * **書いた成分だけ**が、その区間のあいだ毎フレーム上書きされる。
   *
   * - `vx` を書かなければ**それまでの横の勢いがそのまま残る**
   *   （跳んだ勢いを引き継いだまま出す空中技）。`vx: 0` は「勢いを殺す」指定
   * - `vy` を書かなければ重力に任せて落ちる。`vy: 0` は「落下を打ち消して
   *   その場に留まる」指定で、書けば重力より優先される
   */
  motion: [],
  /**
   ...
   * 絵だけを傾ける指定。{deg, pivot}
   *
   * `vfx` と同じで**描画側だけが読む**。判定も移動も傾かないので、
   * 「素材の姿勢が技の趣旨と違う」ときの見た目の補正にだけ使う。
   *
   * - `deg` … 傾き（度）。**正が前のめり**（向いている方へ頭を倒す）。
   *   左を向いていれば自動で反転する
   * - `pivot` … 回す中心の高さ（足元から上へ・ワールド単位）。省略時は腰（100）。
   *   足元を中心に回すと、傾けたぶん体ごと横へ振り出されてしまう
   *
   * **技の途中で傾きを変える指定は用意していない。** 絵は数コマ単位で飛び飛びに
   * 動くので、そこへ 1 ティックずつの連続した回転を重ねると、
   * **コマが変わらない間だけ体が逆向きに滑って、コマが変わった瞬間に戻る**。
   * これが振り下ろしのガクつきになる（絵が 13fps ＝ 4.6 ティックに 1 枚なのに対して、
   * 回転は 60 分の 1 秒ごとに動くため）。傾けっぱなしにしておけば、
   * 技の中で動くのは**絵のコマだけ**になる。
   */
  tilt: null,
  /**
...
export function defineMove(id, raw) {
  ...
  if (move.tilt && !Number.isFinite(move.tilt.deg)) {
    throw new Error(`技 "${id}": tilt には傾ける角度 deg（度）が必要です`);
  }
